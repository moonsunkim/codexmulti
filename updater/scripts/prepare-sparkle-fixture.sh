...
JS
python3 - "$OUTPUT" "$KIND" "$REPO_ROOT/app/Resources/Info.plist" <<'PY'
import json
import pathlib
import plistlib
import socket
...
info.update(CFBundleVersion=old_build, SUFeedURL='https://sparkle.codexmulti.test/appcast.xml',
...
node "$REPO_ROOT/app/scripts/create-appcast.mjs" "$OUTPUT/Feed/CodexMulti.app" "$OUTPUT/Feed/CodexMulti.zip" \
    "$OUTPUT/Feed/appcast.xml" https://sparkle.codexmulti.test/CodexMulti.zip "$OUTPUT/signing-key" "$SPARKLE/bin/sign_update"
